...
class SiswaController extends Controller
{
    ...
    public function submitAbsen(Request $request){           
        dd($request->all());
        // $check = Absensi::where(['siswa_id' => $request->siswa,'kelas_id' => $request->kelas, 'tanggal' => Carbon::now('Asia/Jakarta')->format('Y-m-d')])->get();
        // if(count($check)==0 && $request->status[$i] != "Hadir"){
            $absen = new Absensi;
            $absen->siswa_id = $request->siswa;
        ...
        return redirect()->route("user-dashboard")->with("success", "bla bla bla");
    }
